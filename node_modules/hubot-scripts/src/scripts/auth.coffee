  ...
  robot.respond /@?(.+) (has) (["'\w: -_]+) (role)/i, (msg) ->
    ...
    unless name.toLowerCase() in ['', 'who', 'what', 'where', 'when', 'why']
      ...
      if newRole in user.roles
        msg.send "#{name} already has the '#{newRole}' role."
      else
        if newRole == 'admin'
          msg.send "Sorry, the 'admin' role can only be defined in the HUBOT_AUTH_ADMIN env variable."
        else
          myRoles = msg.message.user.roles or [ ]
          if msg.message.user.name.toLowerCase() in admin.toLowerCase().split(',')
            user.roles.push(newRole)
            msg.send "Ok, #{name} has the '#{newRole}' role."
...
# Remove Roles
  robot.respond /@?(.+) (doesn't have|does not have) (["'\w: -_]+) (role)/i, (msg) ->
    ...
    unless name.toLowerCase() in ['', 'who', 'what', 'where', 'when', 'why']
      ...
      user.roles = user.roles or [ ]
      if newRole == 'admin'
        msg.send "Sorry, the 'admin' role can only be removed from the HUBOT_AUTH_ADMIN env variable."
      else
        myRoles = msg.message.user.roles or [ ]
        if msg.message.user.name.toLowerCase() in admin.toLowerCase().split(',')
          user.roles = (role for role in user.roles when role isnt newRole)
          msg.send "Ok, #{name} doesn't have the '#{newRole}' role."
...
# Individual Roles
  robot.respond /(what role does|what roles does) @?(.+) (have)\?*$/i, (msg) ->
    ...
    msg.send "#{name} has the following roles: #{user.roles} #{isAdmin}."

# Admin Roles
  robot.respond /who has admin role\?*$/i, (msg) ->
    reply = "The following people have the 'admin' role:"
    for user in admin.split(',')
      u = robot.brain.userForId(user)
      reply += "\n- #{u.name}"
    msg.send reply

# Other Roles
# BUG: This will not work, because the roles themselves are not indexed.
# TODO: Robot.brain should index each role as it is added... somehow
  robot.respond /who has (.+) role/i, (msg) ->
    role = msg.match[1]
    if role != 'admin'
      msg.send "I don't have information on #{role} role. Meshach & Sam are still teaching me."
      msg.send "# BUG: This will not work, because the roles themselves are not indexed."
      msg.send "# TODO: Robot.brain should index each role as it is added... somehow"